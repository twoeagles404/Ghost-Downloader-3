        ...
        with zipfile.ZipFile(archive) as zf:
            ...
            for info in files:
                ...
                if safeRoot not in {memberPath, *memberPath.parents}:
                    raise TaskError("Archive contains an unsafe path: {path}", path=info.filename)
    ...
    async def _extractTar(self, archive: Path, outputFolder: Path):
        with tarfile.open(archive, "r:*") as tf:
            ...
            for member in files:
                ...
                if safeRoot not in {memberPath, *memberPath.parents}:
                    raise TaskError("Archive contains an unsafe path: {path}", path=member.name)
                memberPath.parent.mkdir(parents=True, exist_ok=True)
    ...
    async def run(self, reportSpeed, waitForSpeedLimit) -> None:
        ...
        try:
            ...
            for name in self.executableNames:
                ...
                if executable is None:
                    raise TaskError("No executable found after extraction: {name}", name=name)
...
class ChecksumStep(TaskStep):
    ...
    async def run(self, reportSpeed, waitForSpeedLimit) -> None:
        ...
        if not expected:
            raise TaskError("Cannot read verification file: {path}", path=self.sha256File)
        actual = await asyncio.to_thread(self._sha256, Path(self.targetFile))
        if expected != actual:
            raise TaskError("SHA256 verification failed: expected {expected}, actual {actual}", expected=expected, actual=actual)
...
class BinaryInstallStep(TaskStep):
    ...
    async def run(self, reportSpeed, waitForSpeedLimit) -> None:
        ...
        if not path.is_file():
            raise TaskError("Downloaded file not found: {path}", path=str(path))
        if sys.platform != "win32":
