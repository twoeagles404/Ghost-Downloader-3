...
if [[ "${GD3_CN_MIRROR:-0}" == "1" ]]; then
    echo "[run] Building image with a domestic mirror source"
    BUILD_ARGS=(
...
if [[ -n "${HTTP_PROXY:-}" ]]; then
    echo "[run] Detected HTTP_PROXY=$HTTP_PROXY, passing it through to the image build + APK build containers"
    ...
    # docker build uses the host clash via --network=host; BuildKit auto-injects these proxy args into every RUN
...
if [[ -n "${GD3_KEYSTORE:-}" ]]; then
    echo "[run] Detected GD3_KEYSTORE, will sign as a release APK"
...
echo "[run] Building APK inside the container ..."
